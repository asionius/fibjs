...
result_t RadosIoCtx::listImages(v8::Local<v8::Array>& retVal)
{
	result_t hr;
	...
	for (i = 0, cur_name = names; cur_name < names + hr; i++) {
        retVal->Set(i, isolate->NewString((const char*)cur_name));
...
result_t RadosIoCtx::listOids(v8::Local<v8::Array>& retVal, AsyncEvent* ac)
{
	if (ac->isSync())
		return CHECK_ERROR(CALL_E_NOSYNC);
	...
	while ((hr = _rados_objects_list_next(ctx, &entry, NULL)) != -ENOENT)
	{
        ...
        retVal->Set(n++, isolate->NewString(entry));
	}
...
#define RE_SIZE 64
#endif
result_t RadosIoCtx::listOids(exlib::string pattern, v8::Local<v8::Array>& retVal, AsyncEvent* ac)
{
	if (ac->isSync())
		return CHECK_ERROR(CALL_E_NOSYNC);
