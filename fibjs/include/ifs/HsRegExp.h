...
public:
    // HsRegExp_base
    virtual result_t scan(Buffer_base* buff, exlib::string codec, v8::Local<v8::Value>& retVal, AsyncEvent* ac) = 0;
...
public:
    ASYNC_MEMBERVALUE3(HsRegExp_base, scan, Buffer_base*, exlib::string, v8::Local<v8::Value>);
};
}
...
inline void HsRegExp_base::s_scan(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (!cb.IsEmpty()) {
        pInst->acb_scan(v0, v1, cb);
        hr = CALL_RETURN_NULL;
    } else
        hr = pInst->ac_scan(v0, v1, vr);
