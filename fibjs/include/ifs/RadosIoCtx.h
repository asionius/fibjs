...
public:
    ...
    virtual result_t open(exlib::string key, obj_ptr<RadosStream_base>& retVal, AsyncEvent* ac) = 0;
...
public:
    ...
    ASYNC_MEMBERVALUE2(RadosIoCtx_base, open, exlib::string, obj_ptr<RadosStream_base>);
...
inline void RadosIoCtx_base::s_open(const v8::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (!cb.IsEmpty()) {
        pInst->acb_open(v0, cb);
        hr = CALL_RETURN_NULL;
    } else
        hr = pInst->ac_open(v0, vr);
